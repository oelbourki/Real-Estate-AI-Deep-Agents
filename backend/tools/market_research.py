...
class SearchMarketTrendsInput(BaseModel):
    ...
    location: str = Field(
        ..., description="Location to research (e.g., 'San Francisco, CA')"
    )
    timeframe: Optional[str] = Field(
        "1 year",
        description="Timeframe for trends (e.g., '1 year', '6 months', '2 years')",
    )
    topic: Optional[str] = Field(
        None,
        description="Specific topic (e.g., 'home prices', 'rental rates', 'inventory')",
    )
...
def search_market_trends(
    location: str, timeframe: str = "1 year", topic: Optional[str] = None
) -> dict:
    """
    ...
    try:
        ...
        if TAVILY_AVAILABLE and settings.tavily_api_key:
            try:
                ...
                if any(
                    ...
                    for word in ["rising", "increasing", "up", "growth"]
                ):
                    price_trend = "rising"
                elif any(
                    word in all_content.lower()
                    for word in ["falling", "decreasing", "down", "decline"]
                ):
                    price_trend = "falling"
                ...
                logger.warning(
                    f"Tavily API error: {tavily_error}. Falling back to placeholder."
                )
        ...
        if settings.serper_api_key:
            try:
                ...
                logger.warning(
                    f"Serper API error: {serper_error}. Falling back to placeholder."
                )
...
class GetPriceHistoryInput(BaseModel):
    ...
    location: Optional[str] = Field(
        None, description="Location (city, state) for area price history"
    )
    ...
    try:
        ...
        if settings.hasdata_api_key and location:
            try:
                ...
                if "error" not in hasdata_result and hasdata_result.get("success"):
                    ...
                    listings = (
                        data.get("listings", []) if isinstance(data, dict) else []
                    )
                    ...
                    for listing in listings:
                        if listing.get("price") or listing.get("list_price"):
                            price_history.append(
                                {
                                    "date": listing.get("list_date")
                                    or listing.get("date"),
                                    "price": listing.get("price")
                                    or listing.get("list_price"),
                                    "address": listing.get("address")
                                    or listing.get("street_address"),
                                }
                            )
                ...
                logger.warning(
                    f"HasData API error: {hasdata_error}. Trying other methods."
                )
        ...
        if settings.zillow_api_key or settings.hasdata_api_key:
            try:
                ...
                logger.warning(
                    f"Zillow API error: {zillow_error}. Trying other methods."
                )
        ...
        return {"error": f"Failed to get price history: {str(e)}", "address": address}
...
def compare_markets(locations: str) -> dict:
    ...
    try:
        ...
        return {"error": f"Failed to compare markets: {str(e)}", "locations": locations}

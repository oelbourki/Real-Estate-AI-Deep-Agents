...
class ZillowPriceHistoryInput(BaseModel):
    ...
    citystatezip: Optional[str] = Field(
        None, description="City, state, ZIP code (e.g., 'San Francisco, CA 94102')"
    )
    location: Optional[str] = Field(
        None,
        description="Location (city, state) as fallback if citystatezip not provided",
    )
...
def zillow_get_price_history(
    address: str, citystatezip: Optional[str] = None, location: Optional[str] = None
) -> dict:
    """

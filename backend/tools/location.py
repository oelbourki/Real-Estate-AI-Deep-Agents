...
class GeocodeInput(BaseModel):
    ...
    address: str = Field(..., description="The address or place name to geocode (e.g., '123 Main St, San Francisco, CA')")
...
class POISearchInput(BaseModel):
    ...
    key: str = Field(..., description="OSM key, e.g., 'amenity', 'tourism', 'shop', 'building'")
    value: str = Field(..., description="OSM value, e.g., 'restaurant', 'hotel', 'supermarket', 'school'")
...
class NearbyAmenitiesInput(BaseModel):
    ...
    categories: Optional[str] = Field(None, description="Comma-separated categories: 'restaurants', 'schools', 'parks', 'shopping', 'transit'")


@tool("find_nearby_amenities", args_schema=NearbyAmenitiesInput)
def find_nearby_amenities(lat: float, lon: float, radius: int = 1000, categories: Optional[str] = None) -> dict:
    """

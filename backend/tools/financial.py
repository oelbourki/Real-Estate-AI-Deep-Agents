...
class ROIInput(BaseModel):
    ...
    monthly_rent: Optional[float] = Field(None, description="Expected monthly rental income")
    annual_rent: Optional[float] = Field(None, description="Expected annual rental income (alternative to monthly)")
    down_payment: float = Field(0.2, description="Down payment percentage (default: 0.2 for 20%)")
    interest_rate: float = Field(0.065, description="Annual mortgage interest rate (default: 0.065 for 6.5%)")
    loan_term_years: int = Field(30, description="Loan term in years (default: 30)")
    property_tax_rate: float = Field(0.012, description="Annual property tax rate (default: 0.012 for 1.2%)")
    insurance_annual: Optional[float] = Field(None, description="Annual insurance cost")
    maintenance_rate: float = Field(0.01, description="Annual maintenance rate as % of property value (default: 0.01 for 1%)")
    vacancy_rate: float = Field(0.05, description="Vacancy rate (default: 0.05 for 5%)")
    management_rate: Optional[float] = Field(None, description="Property management rate as % of rent (optional)")
...
def calculate_roi(
    ...
    try:
        ...
        insurance = insurance_annual or (purchase_price * 0.003)  # Default 0.3% if not provided
        ...
        total_expenses = annual_mortgage + property_tax + insurance + maintenance + vacancy_loss + management
        ...
        # ROI calculations
        cash_on_cash_roi = (net_annual_income / down_payment_amount * 100) if down_payment_amount > 0 else 0
...
class MortgageInput(BaseModel):
    ...
    interest_rate: float = Field(0.065, description="Annual interest rate (default: 0.065 for 6.5%)")
    loan_term_years: int = Field(30, description="Loan term in years (default: 30)")
    down_payment: Optional[float] = Field(None, description="Down payment amount (optional, for total price calculation)")
...
def estimate_mortgage(
    ...
    try:
        ...
        if down_payment:
            ...
            result["down_payment_percent"] = round((down_payment / total_price * 100), 2)
...
class PropertyTaxInput(BaseModel):
    ...
    tax_rate: Optional[float] = Field(None, description="Annual tax rate (if not provided, uses default 1.2%)")
...
class ComparePropertiesInput(BaseModel):
    ...
    properties: str = Field(..., description="JSON string with property data: [{'price': 500000, 'rent': 3000, ...}, ...]")
        ...
        for i, prop in enumerate(props):
            ...
            if "error" not in roi_result:
                comparisons.append({
                    ...
                    "is_positive_cash_flow": roi_result.get("is_positive_cash_flow")
                })

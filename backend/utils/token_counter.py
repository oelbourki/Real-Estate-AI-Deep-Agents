...
TOOL_DESCRIPTIONS_OVERHEAD = 2000  # Tool descriptions and schemas (2 tools + DeepAgents tools)
SUBAGENT_OVERHEAD = 2500  # Subagent descriptions and prompts (6 subagents with full descriptions)
MEMORY_OVERHEAD = 300  # Memory context (if conversation exists)
FILESYSTEM_OVERHEAD = 1000  # Filesystem context and file listings
DEEPAGENTS_MIDDLEWARE_OVERHEAD = 2500  # DeepAgents middleware overhead (todos, planning, task tool, etc.)
TOTAL_BASE_OVERHEAD = (
...
def validate_token_limit(
    ...
    if estimated > max_tokens:
        ...
        user_message_tokens = estimate_message_tokens(message, user_name, include_overhead=False)
